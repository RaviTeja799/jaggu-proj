...
class DocumentUpdater:
    ...
    def calculate_risk_percentage(
        ...
        # Factor 1: Mandatory status (40%)
        if requirement.mandatory:
            risk_score += 40.0
        else:
            risk_score += 15.0  # Optional requirements still have some risk
        
        # Factor 2: Risk level (30%)
        risk_level_scores = {
            RiskLevel.HIGH: 30.0,
            RiskLevel.MEDIUM: 20.0,
            RiskLevel.LOW: 10.0
        }
        risk_score += risk_level_scores.get(requirement.risk_level, 10.0)
        
        # Factor 3: Framework importance (30%)
        if framework_importance:
            ...
            default_weights = {
                ...
                "CCPA": 0.7,  # Medium-high (California)
